...
logoutButton1.addEventListener("click",(e)=> {
    e.preventDefault()
    const swalWithBootstrapButtons = Swal.mixin({
  customClass: {
    confirmButton: "btn btn-success",
...
AOS.init();
productsDummyData.slice(0,6).forEach((product)=> {
...
function toSingleProduct(pr) {
  window.location.assign(`/pages/single.html?id=${pr}`)
}
/*-----------------------------------------------------------------*/
...
menu_icon.addEventListener("click",()=> {
  ...
  menu_icon.classList.toggle("border","p-5")
